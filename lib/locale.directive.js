...
function install(Vue, options) {
    ...
    exports.formatDate = function (date, format) {
        ...
        if (!lodash_1.isDate(parsedDate))
            throw new TypeError("A date or timestamp must be provided to {{formatDate}}");
        if (lodash_1.isString(format) && format in formats.date)
    ...
    exports.formatMessage = function (message) {
        ...
        if (formatOptions.length === 1 && lodash_1.isPlainObject(formatOptions[0]))
    ...
    lodash_1.each(helpers, function (helper, name) {
        Vue.filter(lodash_1.kebabCase(name), helper);
    ...
    Vue.filter("format-percent", {
        read: function (val, fractionDigits) {
            return exports.formatNumber(val == undefined || val === "" ? 0 : lodash_1.clamp(val / 100, 0, 1), {
                style: "percent",
